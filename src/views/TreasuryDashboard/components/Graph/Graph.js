...
export const ProtocolOwnedLiquidityGraph = () => {
  ...
  return (
    <Chart
      ...
      dataKey={["treasuryLionDaiPOL"]}
      bulletpointColors={bulletpoints.pol}
      infoTooltipMessage={tooltipInfoMessages.pol}
      headerText="Protocol Owned Liquidity LION-DAI"
      expandedGraphStrokeColor={theme.palette.graphStrokeColor}
      headerSubText={`${data && trim(data[0].treasuryLionDaiPOL, 2)}% `}
      stopColor={[["rgba(128, 204, 131, 1)", "rgba(128, 204, 131, 0)"]]}
    />
  );
};

export const LIONStakedGraph = () => {
  const theme = useTheme();
  const { data } = useTreasuryMetrics({ refetchOnMount: false });
  ...
  const staked =
    ...
    data
      .map(metric => ({
        staked: (metric.sLionCirculatingSupply / metric.lionCirculatingSupply) * 100,
