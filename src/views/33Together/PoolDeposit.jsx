...
const slionImg = getTokenImage("slion");
...
export const PoolDeposit = props => {
  ...
  const poolAllowance = useSelector(state => {
    return state.account.pooling && state.account.pooling.slionPool;
  });
  ...
  const setMax = () => {
    const value = parseFloat(slionBalance);
    setQuantity(value);
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      t`Deposit sLION to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sLION is withdrawn.`,
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
              ...
              <OutlinedInput
                ...
                startAdornment={
                  <InputAdornment position="start">
                    <div className="logo-holder">{slionImg}</div>
                  </InputAdornment>
                }
                labelWidth={0}
                endAdornment={
            ...
            {address && hasAllowance("slion") ? (
              <Button
                ...
                style={{ margin: "5px" }}
              >
                {txnButtonText(pendingTransactions, "pool_deposit", t`Deposit sLION`)}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                <Trans>
                  Depositing {quantity} sLION will increase odds of winning to 1 in {newOdds}
                </Trans>
                &nbsp;
              </Typography>
            </Box>
          )}
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sLION in during testing */}
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(slionBalance)} sLION</>
                )}
              </Typography>
            </div>
          </div>
