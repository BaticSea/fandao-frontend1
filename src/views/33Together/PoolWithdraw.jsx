        ...
        {!address ? (
          ...
          <Box className="withdrawal-container">
            <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
              ...
              >
                {exitFee > 0
                  ? txnButtonText(pendingTransactions, "pool_withdraw", t`Withdraw Early & pay` + exitFee + " sLION")
                  : txnButtonText(pendingTransactions, "pool_withdraw", t`Withdraw sLION`)}
                {/* Withdraw sLION */}
              </Button>
            </Box>
            {newOdds > 0 && quantity > 0 && (
              <Box padding={1}>
                <Typography color="error" variant="body2">
                  <Trans>
                    Withdrawing {quantity} sLION reduces your odds of winning to 1 in {newOdds}
                  </Trans>
                  &nbsp;
                </Typography>
              </Box>
