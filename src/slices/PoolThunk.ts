...
import { AwardAbi2, PrizePoolAbi, PrizePoolAbi2, SLION } from "src/typechain";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const slionContract = new ethers.Contract(addresses[networkID].SLION_ADDRESS, ierc20Abi, signer) as SLION;

    let approveTx;
    let depositAllowance = await slionContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
    ...
    depositAllowance = await slionContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
