...
import { IERC20, WsFAN } from "src/typechain";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const wsfanContract = new ethers.Contract(
      addresses[networkID].WSFAN_ADDRESS as string,
      ierc20ABI,
      signer,
    ) as IERC20;
    let approveTx;
    let wrapAllowance = await sfanContract.allowance(address, addresses[networkID].WSFAN_ADDRESS);
    let unwrapAllowance = await wsfanContract.allowance(address, addresses[networkID].WSFAN_ADDRESS);

    try {
      if (token === "sfan") {
        // won't run if wrapAllowance > 0
        approveTx = await sfanContract.approve(
          addresses[networkID].WSFAN_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei"),
        );
      } else if (token === "wsfan") {
        approveTx = await wsfanContract.approve(
          addresses[networkID].WSFAN_ADDRESS,
    ...
    return dispatch(
      fetchAccountSuccess({
        wrapping: {
          fanWrap: +wrapAllowance,
          fanUnwrap: +unwrapAllowance,
        },
      }),
    );
  },
...
export const changeWrap = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    const signer = provider.getSigner();
    const wsfanContract = new ethers.Contract(addresses[networkID].WSFAN_ADDRESS as string, wsFAN, signer) as WsFAN;
    ...
    try {
      if (action === "wrap") {
        uaData.type = "wrap";
        wrapTx = await wsfanContract.wrap(ethers.utils.parseUnits(value, "gwei"));
      } else {
        uaData.type = "unwrap";
        wrapTx = await wsfanContract.unwrap(ethers.utils.parseUnits(value));
      }
